    ...
    <?php
        if($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if(isset($_GET["name"])){
                ...
                echo '<label for="name">タイトル： </label><input type="text" id="name" name="name" value="'.$_GET["name"].'"><br>';
            }else{
                echo '<label for="name">タイトル： </label><input type="text" id="name" name="name"><br>';
            }
            if(isset($_GET["date"])){
                $ip .= "<input type='hidden' name='date' value='". $_GET["date"] ."' form='gf'>";
                $q_date = $_GET["date"];
                echo '<label for="date">日付　：</label><input type="date" id="date" name="date" value="'.$_GET["date"].'"><br>';
            }else{
                echo '<label for="date">日付　：</label><input type="date" id="date" name="date"><br>';
            }
            if(isset($_GET["range"])){
            ...
            $stmt = $db_link->prepare("SELECT * FROM histories WHERE name LIKE ? AND detail LIKE ? AND ABS(DATEDIFF(date, ?)) <= ? AND status < 2;");
            ...
            {
                echo "<p>見つかりません...</p>";
                echo "<p><a href='../goods_reg'>事実ならこちらで登録しましょう！</a></p>";
            }    
            $db_link = null;
            $stmt = null;
        }
